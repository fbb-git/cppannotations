...
file descriptor and the  tt(Child) object's address:
        verbinsert(CHILD)(examples/monitor/createnewchild.cc)
...
members:
        verbinsert(FINDIMP)(examples/monitor/monitor.h)
...
implementation:
        verbinsert(KILL)(examples/monitor/monitor.cc)
...
object is destroyed and its pointer is removed from tt(d_child):
        verbinsert(STOP)(examples/monitor/stopchild.cc)
...
process:
        verbinsert(SEND)(examples/monitor/sendchild.cc)
...
tt(d_child). Then the program itself ends:
        verbinsert(EXIT)(examples/monitor/monitor.cc)
    )
    The program's tt(main) function is simple and needs no further comment:
        verbinsert(MAIN)(examples/monitor/main.cc)
